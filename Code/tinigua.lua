...
local function handlePlayButtonEvent(event)
    if ("ended" == event.phase) then
        composer.removeScene("tinigua",false)
...
local function handleOptButtonEvent(event)
    if ("ended" == event.phase) then
        composer.removeScene("tinigua",false)
        composer.gotoScene("tinigua_options", {effect = "crossFade", time = 500})
    end
end
...
function scene:create(event)
    ...
    local mbgImage = {type="image", filename="TiniguaForest.png"} 
